...
- **Модель пространства:** **одна глобальная общая лента на всех.** Никаких
  комнат/подписок — кто установил приложение, тот в едином пространстве, видит
  все посты и постит в общую «стену». Реплицируется через один глобальный
  MQTT-топик `b2r/global/v1` (retained snapshot, merge по LWW).
