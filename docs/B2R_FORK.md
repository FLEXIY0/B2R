...
- ⚠️ Сборку KMP/Android-проекта нельзя верифицировать в текущем окружении
  (нет Android SDK). Правки делаются типобезопасно; компиляцию нужно проверить
  в окружении со сборкой.
