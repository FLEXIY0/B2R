...
/**
 * b2r fork (point 1, stage 2): a tiny MQTT 3.1.1 control-packet codec.
 *
 * Just enough of the protocol to use public brokers as a retained "mailbox"
 * over a WebSocket, mirroring the owner's `todo` sync (which also hand-rolled a
 * minimal MQTT-over-WebSocket client instead of pulling a library). QoS 0 only,
 * clean session, no authentication.
 *
 * NOTE: the byte framing here is compile-checked but not yet exercised on a
 * device — treat it as experimental until verified against a live broker.
 */
internal object MqttWireFormat {

    private const val TYPE_CONNECT = 0x10
    private const val TYPE_PUBLISH = 0x30
    private const val TYPE_SUBSCRIBE = 0x82 // SUBSCRIBE requires the 0x02 flags bit
    private const val TYPE_PINGREQ = 0xC0
    private const val TYPE_DISCONNECT = 0xE0

    private const val RETAIN_FLAG = 0x01
    private const val PROTOCOL_LEVEL_3_1_1 = 0x04
    ...
    fun connect(clientId: String, keepAliveSeconds: Int = DEFAULT_KEEP_ALIVE_SECONDS): ByteString {
        val variableHeader = encodeString("MQTT") +
            byteArrayOf(PROTOCOL_LEVEL_3_1_1.toByte()) +
            byteArrayOf(CLEAN_SESSION.toByte()) +
            byteArrayOf((keepAliveSeconds ushr 8).toByte(), (keepAliveSeconds and BYTE_MASK).toByte())
    ...
    fun subscribe(packetId: Int, topicFilter: String): ByteString {
        ...
        val payload = encodeString(topicFilter) + byteArrayOf(0x00) // requested QoS 0
        return packet(TYPE_SUBSCRIBE, variableHeader + payload)
    }
    ...
    fun pingReq(): ByteString = packet(TYPE_PINGREQ, ByteArray(0))
    ...
    /** Parse an incoming control packet; returns the message for a PUBLISH, else null. */
    fun parsePublish(frame: ByteString): PublishMessage? {
        ...
        var index = 1
        // skip the remaining-length varint
        ...
        index += topicLength
        // QoS 0 => no packet identifier; the rest is the application payload.
        return PublishMessage(topic = topic, payload = data.copyOfRange(index, data.size))
    }

    private const val DEFAULT_KEEP_ALIVE_SECONDS = 30
}
